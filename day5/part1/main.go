...
func (game *Game) Run() {
	for _, instruction := range game.instructions {
		...
		for index := 1; index <= instruction.amount; index++ {
			value := game.stacks[fromIndex][fromLen - index]
			game.stacks[toIndex] = append(game.stacks[toIndex], value)
		}

		game.stacks[fromIndex] = slices.Delete(game.stacks[fromIndex], fromLen - instruction.amount, fromLen)
	}
